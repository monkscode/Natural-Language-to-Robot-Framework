...
describe('GeneratePage — a completed execution', () => {
  ...
  it('does not carry Run #1’s sentence into a Run #2 failure whose event has no failure_sentence key', async () => {
    allowFeedbackMount()
    const failLogs = [
      'Test: Checkout flow - FAIL',
      'Error: Element not found: #submit',
    ...
    mockStreamSSE.mockImplementation(async (path, _body, onEvent) => {
      ...
      if (execCalls === 1) {
        onEvent({
          status: 'complete',
          result: { report_html: '/reports/RUN-10/log.html', logs: failLogs },
          test_status: 'failed',
          failure_sentence: 'The submit button could not be found on the page.',
        })
      } else {
        // Run #2 fails too, but its event carries no failure_sentence key at all
        // (e.g. an older backend, or a failure path failure_sentences.py can't classify).
        onEvent({ status: 'complete', result: { report_html: '/reports/RUN-10/log.html', logs: failLogs }, test_status: 'failed' })
      }
    })
    renderPage()
    typeQuery()
    ...
    expect(await screen.findByText('Open the detailed log to see exactly which step went wrong.')).toBeInTheDocument()
    expect(screen.queryAllByText('The submit button could not be found on the page.')).toHaveLength(0)
  })

  it('does not carry Run #1’s sentence into a Run #2 failure whose event sends failure_sentence: null', async () => {
    allowFeedbackMount()
    const failLogs = [
      'Test: Checkout flow - FAIL',
      'Error: Element not found: #submit',
    ...
    mockStreamSSE.mockImplementation(async (path, _body, onEvent) => {
      ...
      if (execCalls === 1) {
        onEvent({
          status: 'complete',
          result: { report_html: '/reports/RUN-11/log.html', logs: failLogs },
          test_status: 'failed',
          failure_sentence: 'The submit button could not be found on the page.',
        })
      } else {
        // Run #2 fails too; its event carries the key but with an explicit null —
        // failure_sentences.py's classifier can return None for an unrecognized message.
        onEvent({
          status: 'complete',
          result: { report_html: '/reports/RUN-11/log.html', logs: failLogs },
          test_status: 'failed',
