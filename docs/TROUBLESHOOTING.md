...
| **First **Run Test** fails with `runner-exec unreachable: ... Read timed out`** | Your tests execute in a `test-runner` container that is pulled on demand, and it is ~1.9 GB — the first run waits on that download. Pull it up front so this cannot happen: `docker pull monkscode/nlrf:test-runner-develop` (match the tag suffix in your root `.env`). If you already hit it, the download is still finishing in the background — wait for it to complete and click **Run Test** again. Watch progress with `docker compose logs -f runner-exec`. |
