...
# Shared by test_org_membership_integration.py and test_org_db.py — it lives
# here rather than in either of them because both need the same stubs.
def ensure_stub_learning_tables(conn) -> None:
    """Create minimal stand-in tables for the 3 real learning tables, inside
    the isolated auth_test schema, if they don't already exist.

    auth_isolated_schema (this module) sets search_path=auth_test with NO
    ,public fallback, and the real bootstrap
    (crew_ai.optimization.pg_schema.ensure_schema) cannot run here: it
    creates vector(384) columns whose vector type is pinned to the public
    schema, which is not on this suite's search_path. These stubs carry only
