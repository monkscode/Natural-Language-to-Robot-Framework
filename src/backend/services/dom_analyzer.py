...
class DOMAnalyzer:
    ...
    def generate_locator_for_element(self, element_info: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if unique_attrs:
                attr_selector = ''.join([f'[{k}="{v}"]' for k, v in unique_attrs.items()])
    ...
    def _find_element_by_locator(self, soup: BeautifulSoup, locator: str) -> Optional[Tag]:
        ...
        try:
            ...
            logger.warning(f"Failed to find element by locator '{locator}': {e}")
            return None
    ...
    def _find_unique_attributes(self, element: Tag) -> Dict[str, str]:
        ...
        priority_attrs = ['data-testid', 'data-test', 'role', 'type', 'placeholder']
        ...
        try:
            ...
            for part in parts:
                ...
                if '[' in part and ']' in part:
                    ...
                    # Simple tag name
                    for elem in current_elements:
                        next_elements.extend(elem.find_all(part, recursive=False))
