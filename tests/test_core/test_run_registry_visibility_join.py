...
@pytest.mark.xfail(
    raises=AssertionError, strict=True, reason=(
        ...
        "disagree for that caller. THE POPULATION IS WIDER THAN THE "
        "ACCEPTANCE WAS TAKEN ON, and is recorded here rather than "
        "re-opened (measured 2026-09-12, P2 whole-branch review, owner "
        "decision D11): it is not only the admin themselves once seen as an "
        "ordinary member. EVERY org_admin of the org the admin's token "
        "carried reaches it immediately, with no demotion at all -- their "
        "filter user_id is None, so the re-run is in their org's Activity "
        "while _VISIBLE_TEST_SQL still drops the unowned test. Measured "
        "with its own known negative: a DIFFERENT plain member of that org "
        "reaches neither side and sees no divergence. The acceptance is "
        "RE-AFFIRMED on that wider population. Left failing on purpose as the regression "
