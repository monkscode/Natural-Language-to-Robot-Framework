...
Depends on: core/secret_redaction.py.
"""
...
# generic PERMISSION_DENIED catch. Each entry is (predicate, message builder).
_RULES = (
    ...
    (lambda t: "ratelimiterror" in t.lower()
               or "429" in t
               or "quota exceeded" in t.lower()
               or "resource exhausted" in t.lower(), _rate_limited),
    (lambda t: "PERMISSION_DENIED" in t or "permission denied" in t.lower(), _vertex_permission),
)
