...
def test_same_org_admin_peer_can_submit_feedback(client):
    ...
    o_org = orgs.create_team_org("Feedback QA", str(owner["id"]))  # seats owner as org_admin
    ...
    # peer whose token never authenticated would pass a bare `!= 403` without
    # the org path ever executing — the gate would go green while proving nothing.
