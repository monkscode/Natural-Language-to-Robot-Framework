"""
JWT minting + FastAPI auth dependencies (coexistence-aware).
...
- require_user       — requires a token only when settings.AUTH_ENFORCED; else
                       permissive (returns the user if a token is present, None
                       if absent) so the legacy unauthenticated UI keeps working.
...
enforced (a broken token is a client error regardless of the coexistence flag).
...
def require_user(
    ...
    """Enforced only when settings.AUTH_ENFORCED; otherwise permissive."""
    user = _decode_to_user(credentials)
...
def require_admin(
    ...
    """Admin guard. Permissive during coexistence (AUTH_ENFORCED=False) so the
    legacy learning/metrics dashboards keep working; strict at cutover."""
    user = _decode_to_user(credentials)
