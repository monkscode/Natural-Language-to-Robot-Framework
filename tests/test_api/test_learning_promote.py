...
shared with another customer — global hints are exclusively admin-CREATED
(POST /hints, generic guidance only), never promoted from org data. The route
was removed to make that guarantee structural.

These tests pin the removal: the route must not exist, and no hint/anchor
state may change from calling the old path.
"""
...
def _hint_and_anchor_state(dsn, hint_id):
    ...
    try:
        hint = conn.execute(
            "SELECT is_shared FROM nl_feedback_corrections WHERE id = ?",
            (hint_id,),
        ).fetchone()
        anchor = conn.execute(
...
def test_promote_route_is_removed(promote_client, seeded_hint_id, api_pg_em):
    ...
    assert hint is not None and hint["is_shared"] == 0, (
        "hint must remain org-private after hitting the removed promote path"
    )
